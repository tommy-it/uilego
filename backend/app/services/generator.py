...
def _generate_find_element_code(element: Element, action_desc: str) -> List[str]:
    ...
    if len(standard_locators) == 1 and not coord_locators:
        ...
        lines.append(f"        el = _scroll_to_find(driver, {code})")
        lines.append(f"        if el is None and '_tap_coord' not in locals():")
        lines.append(f"            raise Exception('❌ [{element.name}] 定位器失败（已滚动查找），请检查元素配置')")
        return lines
    ...
    lines.append(f"            el = _scroll_to_find(driver, _loc)")
...
def _generate_step_code(step: TestStep, element: Optional[Element], index: int) -> List[str]:
    ...
    if action == "tap":
        if element:
            ...
            lines.append(f"        elif '_tap_coord' in locals():")
...
def generate_pytest_script(
    ...
    lines.append("")
    lines.append("")
    lines.append("def _scroll_to_find(drv, locator, max_scrolls=4):")
    lines.append('    """滚动查找元素，找不到时自动滚动屏幕"""')
    lines.append("    # 先尝试直接查找")
    lines.append("    try:")
    lines.append("        return WebDriverWait(drv, 3).until(EC.presence_of_element_located(locator))")
    lines.append("    except Exception:")
    lines.append("        pass")
    lines.append("    # 向下滚动查找")
    lines.append("    size = drv.get_window_size()")
    lines.append("    sx = size['width'] // 2")
    lines.append("    sy = int(size['height'] * 0.7)")
    lines.append("    ey = int(size['height'] * 0.3)")
    lines.append("    for i in range(max_scrolls):")
    lines.append("        drv.swipe(sx, sy, sx, ey, 500)")
    lines.append("        time.sleep(0.5)")
    lines.append("        try:")
    lines.append("            return drv.find_element(*locator)")
    lines.append("        except Exception:")
    lines.append("            pass")
    lines.append("    # 没找到，回滚到顶部")
    lines.append("    for i in range(max_scrolls + 2):")
    lines.append("        drv.swipe(sx, ey, sx, sy, 300)")
    ...
    lines.append(f'    options.no_reset = True')
    lines.append(f'    options.auto_grant_permissions = True')
    lines.append(f'    options.new_command_timeout = 300')
    lines.append(f'    drv = webdriver.Remote("{appium_url}", options=options)')
    lines.append('    drv.implicitly_wait(5)')
    lines.append(f'    # 按 HOME + 重启应用，确保从首页开始')
    lines.append(f'    drv.press_keycode(3)  # HOME 键回到桌面')
    lines.append('    import time; time.sleep(0.5)')
    lines.append(f'    try:')
    lines.append(f'        drv.terminate_app("{app_package}")')
    lines.append(f'    except Exception:')
    lines.append(f'        pass')
    lines.append(f'    time.sleep(0.5)')
    lines.append(f'    drv.activate_app("{app_package}")')
